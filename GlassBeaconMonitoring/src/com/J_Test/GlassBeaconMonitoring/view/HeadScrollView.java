package com.J_Test.GlassBeaconMonitoring.view;
...
public class HeadScrollView extends ScrollView implements SensorEventListener {
	...
	public void onSensorChanged(SensorEvent event) {
		...
		float z = orientation[0], // see
									// https://developers.google.com/glass/develop/gdk/location-sensors/index
		x = orientation[1], y = orientation[2];

		if (mStartX == 10)
			mStartX = x;

		float mEndX = mStartX
				- (getChildAt(0).getHeight() - getHeight() * 0.5F) / VELOCITY;

		int prior = getScrollY(), pos = (int) ((mStartX - x) * VELOCITY);
